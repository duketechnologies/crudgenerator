...
To get started, you should add the `Duke/crud-generator` Composer dependency to your project:
```
composer require Duke/crud-generator --dev
```
Once the package is installed, you should register the `Duke\CrudGenerator\CrudGeneratorServiceProvider` service provider. Normally, Laravel 5.5+ will register the service provider automatically.
...
public function register()
{
    if ($this->app->environment() == 'local') {
        $this->app->register('Duke\CrudGenerator\CrudGeneratorServiceProvider');
    }
}
```
